...
**Why the driver map is "course-up" (GPS), never compass-up:** The map rotates to the driver's smoothed GPS direction-of-travel only while actually moving, and holds steady when stopped. The phone's magnetometer rotates ONLY the position arrow, never the map. Earlier the HomeScreen fed the raw magnetometer into the map camera, so the slightest turn/shake of the phone spun the whole map (compass is noisy in a car — metal, speakers, magnetic mounts) and made it impossible to orient while driving to a client. Helpers live in `packages/shared/src/utils/geo.ts` (`advanceCourseUp`, distance-gated + low-pass smoothed); the active-order screen derives heading from a Kalman velocity filter and holds it below ~9 km/h so slow village crawling doesn't swing the view.
